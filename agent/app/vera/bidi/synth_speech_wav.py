#!/usr/bin/env python3
"""
Phase D / M1.2b spike — text → WAV PCM16 mono 8 kHz.

Throwaway. Generates the synthetic-voice inputs for M1.2b (banking) and
M1.2c (salud) tests. Two backends:
...
Install hints:
    piper:    pip install piper-tts   (in tmp/spike-sip-venv for this spike)
...
Example (M1.2b banking happy path, piper):
...
def _synth_piper(text: str, tmp_wav: Path) -> None:
    if not shutil.which("piper"):
        raise SystemExit(
            "piper not found on PATH. Activate the spike venv "
            "(tmp/spike-sip-venv) or pip install piper-tts."
        )
